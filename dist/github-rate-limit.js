export class GitHubRateLimitError extends Error {
    ...
    constructor(retryAfterSeconds, resetAt) {
        const retryText = retryAfterSeconds !== undefined
            ? ` Retry after about ${retryAfterSeconds} seconds.`
            : '';
        const resetText = resetAt ? ` GitHub reset time: ${resetAt}.` : '';
        super(`GitHub API rate limit exceeded.${retryText}${resetText} Set GITHUB_TOKEN for a higher request limit.`);
...
function getRetryAfterSeconds(headers, now = Date.now()) {
...
export function toGitHubRateLimitError(error, now = Date.now()) {
...
export function formatGitHubApiError(error) {
    const rateLimitError = toGitHubRateLimitError(error);
    if (rateLimitError)
        return rateLimitError.message;
    return error instanceof Error ? error.message : String(error);
}
export async function runGitHubRequest(request, options = {}) {
    ...
    for (let attempt = 0;; attempt += 1) {
        try {
            ...
            const retryAfterMs = rateLimitError.retryAfterSeconds !== undefined
...
//# sourceMappingURL=github-rate-limit.js.map